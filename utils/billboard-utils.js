...
const getBillboardClass = (billboardData) => {
  ...
  for (let i = 0; i < billboardData.length; i++) {
    if (billboardData[i].class === 'digital') {
      if (
        _.has(
          billboardClassObject.billboardClass.digital,
          billboardData[i].status
        )
      ) {
      ...
      if (
        _.has(
          billboardClassObject.billboardClass.static,
          billboardData[i].status
        )
      ) {
...
const getBillboardTypes = (billboardData) => {
  let billboardTypesObject = {
    billboardTypes: _.countBy(
      _.sortBy(billboardData, 'type'),
...
const getBillboardStateStatus = (billboardData) => {
  const groupStates = _.groupBy(
    _.sortBy(billboardData, 'state'),
  ...
  for (let key in groupStates) {
    let billboardStateStatusShape = {
      state: key,
      ..._.countBy(groupStates[key], (billboard) =>
    ...
    let groupTypes = _.groupBy(
      _.sortBy(groupStates[key], 'type'),
      (billboard) => billboard.type
    );
    for (let key in groupTypes) {
      billboardStateStatusShape.types.push({
        type: key,
        ..._.countBy(groupTypes[key], (billboard) =>
          billboard.status === 'active'
            ? 'active'
            : billboard.status === 'inactive'
            ? 'inactive'
    ...
    billboardStateStatusObject.billboardStatesStatus.push(
      billboardStateStatusShape
    );
  }
